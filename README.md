...
你的工作能力要求文档非常清晰且详细，涵盖了公司工作流程中所需的核心技术和工具。以下是对你文档的一些补充和优化建议，以帮助进一步完善内容：

## **基础技术栈**

本章节讨论了在公司开展工作之前所有角色都应当掌握的技术与工具。

### **Github Desktop**

在工作中，我们使用 `Github` 进行代码和文档托管。因此，必须掌握 `Github Desktop` 的基本使用方法，包括克隆、提交、拉取、分支等操作。同时，还应当掌握 `Git` 使用的基本规范，以下是我们提倡的使用规范：

- **Commit Message** 应当简洁明了，能够清晰地表达本次提交的内容，不应当出现无意义的 Commit Message。
...
- 请用 Markdown 结合插件满足诸如表格、代码块、公式、思维导图等任何需求。善用插件，提高工作效率。
...
- **禁止使用 AI 进行创作**：任何涉及创作的工作（如文本创作、设计创作等）**禁止**使用 AI 工具。
- **谨慎使用 AI 进行数据处理**：由于 AI 工具的原理限制，推理结果难以保证数据的准确性。必须在人工的基础上进行验证。
- **代码生成**：AI 工具可以用于代码生成，但应当注意代码的质量和可读性。

### **Teambition**

使用 Teambition 参与项目管理，即时更新任务进度，及时反馈问题，保持团队的协作性。项目绩效评估将会参考 Teambition 的任务完成情况。

### **企业微信**

使用企业微信进行工作沟通，及时回复消息，保持沟通畅通。减少非工作时间沟通或普通微信沟通，保持工作和生活的分离。
...
我们使用 `MediaWiki` 作为公司的知识库，因此应当掌握 `MediaWiki` 的基本使用方法，包括但不限于：
...
通过掌握以上技术和工具，团队成员可以高效协作，确保工作流程的规范性和数据的一致性。如果有任何疑问或需要进一步的学习资源，请随时向团队中的技术负责人咨询！ 😊
