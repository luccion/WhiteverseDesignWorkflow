...
- **分支管理**：
  - 不复刻(`fork`)分支，而是克隆(`clone`)原仓库，否则管理人员无法维护和监督分支内容。
  - 不在主干（如 `main` 或 `master`）上进行开发，应当在功能分支上进行开发。
...
**必须**使用 `VSCode` 作为主要的文本/代码编辑器，并且根据库的需求和建议安装相应的插件。基本插件如下：

- **Edit csv**：编辑 CSV 文件。
- **Rainbow CSV**：高亮显示 CSV 文件内容。
- **Markdown AllInOne**：增强 Markdown 预览功能。
- **MarkdownLint**：检查 Markdown 语法规范。
- **Prettier - Code formatter**：自动格式化代码。
- **Color Highlight**：高亮显示颜色代码。

_P.S. 禁止使用 NotePad++、Typora 等不同规范的编辑器。_
